  ...
  start() {
    ...
    server.on("message", async function (msg, rinfo) {
      ...
      if (msg) {
        ...
        // Init Objects
        if (deveiceInfo.devName == 'em06') { // Currently only supported, We will gradually support different models of devices in the future
          new base(that.adapter, that.objectHelper, deveiceInfo)
        }
      }
    });
    // Server listening
