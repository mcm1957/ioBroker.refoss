        ...
        if (deviceStates) {
          for (const stateId in deviceStates) {
            ...
            if (state.http?.http_namespace) {
              ...
              //   http_uuid: this.uuid,
              //   http_ip: this.ip,
              //   http_namespace: state.http?.http_namespace,
              //   http_payload: state.http?.http_payload
              // }
              // const httpAck = new SelfHttp(this.adapter, httpInfo)
              // this.httpData = httpAck.httpResponseData[state.http?.http_get_ack]
              // const that = this
              // this.timer && clearInterval(this.timer)
              // this.timer = setInterval((function target() {
              //   that.adapter.log.warn(`setInterval---`);
              //   const httpAck = new SelfHttp(that.adapter, httpInfo)
              //   this.httpData = httpAck.httpResponseData[state.http?.http_get_ack]
              //   return target
              // }), 15000) // 15s Polling
            }
